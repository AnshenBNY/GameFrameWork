...
public class ShootBehaviour : GenericBehaviour
{
	...
	public Color killMarkerColor = new Color(1f, 0.45f, 0.2f, 1f);
	public float hitMarkerDuration = 0.08f;
	public float killMarkerDuration = 0.16f;
	...
	private float killMarkerTimer = 0f;
	...
	private void Update()
	{
		...
		killMarkerTimer = Mathf.Max(0f, killMarkerTimer - Time.deltaTime);
	}
	...
	private void ShootWeapon(int weapon, bool firstShot = true)
	{
		...
		// Target was hit.
		if (CombatRaycastUtility.TryFirstHit(ray, 500f, shotMask, transform.root, out RaycastHit hit))
		{
	...
	private void OnGUI()
	{
		if (!showHitMarker || (hitMarkerTimer <= 0f && killMarkerTimer <= 0f))
		{
			return;
		}

		if (hitMarkerStyle == null)
		{
		...
		Rect rect = new Rect((Screen.width - 80f) * 0.5f, (Screen.height - 80f) * 0.5f, 80f, 80f);
		if (killMarkerTimer > 0f)
		{
			hitMarkerStyle.normal.textColor = killMarkerColor;
			GUI.Label(rect, "X", hitMarkerStyle);
			return;
		}

		hitMarkerStyle.normal.textColor = hitMarkerColor;
